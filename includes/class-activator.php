...
final class Activator {
	...
	public static function activate() {
		...
		self::install_network_defaults();

		flush_rewrite_rules();
	}

	/**
	 * Seed the network defaults, but only where they mean something.
	 *
	 * The `multisite` feature is gated on the `multisite` capability: on a single-site
	 * install this is a no-op and the feature reports itself blocked in the admin.
	 *
	 * @return void
	 */
	public static function install_network_defaults() {
		$capabilities = \DashWoo\Capabilities\Capabilities::instance();

		if ( ! $capabilities->enabled( 'multisite' ) ) {
			return;
		}

		update_site_option( 'dashwoo_network_defaults', array( 'seeded_at' => time(), 'version' => DASHWOO_VERSION ) );
	}
